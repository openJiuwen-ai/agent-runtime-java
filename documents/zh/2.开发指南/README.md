...
| 5 | [开发 Agent Service](开发Agent Service.md) → [多 Agent 实例托管](多Agent实例托管.md) → [Adapters 与 Handler](Adapters与Handler.md) → [生命周期与探针](生命周期与探针.md) | 定制业务镜像、单进程多实例托管 |
| 6 | [外部服务](开发与扩展/外部服务.md) | MCP、A2A Remote、Sandbox 出站（需要时） |
| 7 | [A2A 与平台边界](A2A/平台边界.md) → [A2A 开发指导](A2A/开发指导.md) | 需要 A2A 时；多实例入口见 [多 Agent 实例托管](多Agent实例托管.md) |
...
- 本仓 **已交付进程内 A2A**（Agent Card、JSON-RPC）和单进程多 Agent 实例托管；**平台级**多 Agent 路由与独立 `a2a_service` 进程仍属规划，见 [A2A 与平台边界](A2A/平台边界.md)。
...
| [开发与扩展](开发与扩展/README.md) | 镜像、Handler、实例托管、生命周期、外部 egress | [开发 Agent Service](开发Agent Service.md)、[多 Agent 实例托管](多Agent实例托管.md)、[Adapters 与 Handler](Adapters与Handler.md)、[生命周期与探针](生命周期与探针.md)、[外部服务](开发与扩展/外部服务.md) |
