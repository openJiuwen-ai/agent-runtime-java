...
本文描述 **openJiuwen Agent Distributed Runtime** 的整体逻辑架构（跨语言、跨进程的目标蓝图）。Java 本仓实现细节见 [架构概述](架构概述.md)。
...
A2A：`A2aJsonRpcController` → `A2AProtocolAdapter` → 同上 Orchestrator 链。详见 [A2A 与平台边界](A2A与平台边界.md)。
...
详见 [A2A 开发指导](A2A开发指导.md)。平台 A2A（规划）见 [A2A 与平台边界](A2A与平台边界.md)。

---

## 延伸阅读

- 当前模块与调用链：[架构概述](架构概述.md)
