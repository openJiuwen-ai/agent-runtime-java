...
当前仓库 **`agent-runtime-java`** 主要交付 **Agent Server（② Service）** 与 **Agent-Core（① 子模块）**；**Agent Runtime Manager**、完整 **Agent Gateway** 等将随 **`manager/*`** 等模块逐步补齐。平台级 Manager 模块树与运行模型将在 **`manager/*` 入仓后**于本仓库 `documents/` 补充说明。
...
| **Agent-Core** | Team / Evolving / Harness / Core | `vendor/agent-core-java` |
...
| 外部能力 | 集成方式 | 本仓现状 |
| --- | --- | --- |
| **LLM** | Core `foundation.llm` | ✅ Core submodule |
...
├── vendor/agent-core-java          → Agent-Core
...
- Agent Core 能力：[vendor/agent-core-java/documents/zh/2.开发指南/README.md](../../vendor/agent-core-java/documents/zh/2.开发指南/README.md)
