...
MAIN_CLASS="com.openjiuwen.service.demo.example.queryl1.QueryL1RestExample"
...
Run these commands from agent-runtime-java/service first:
  ...
  javac -d agent-service-demo/target/example-classes -cp "\$EXAMPLE_CP" agent-service-demo/example/query/QueryRestExample.java agent-service-demo/example/query/QueryL1RestExample.java
EOF
    exit 1
  fi
}
