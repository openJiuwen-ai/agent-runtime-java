...
MAIN_CLASS="com.openjiuwen.service.demo.example.healthl1.HealthL1ProbeExample"
EXAMPLE_CLASSES="$SERVICE_DIR/agent-service-demo/target/example-classes"
APP_CP_FILE="$SERVICE_DIR/agent-service-app/target/app.classpath"
DEMO_CP_FILE="$SERVICE_DIR/agent-service-demo/target/example.classpath"
...
require_classpath() {
  if [[ ! -d "$EXAMPLE_CLASSES" || ! -f "$APP_CP_FILE" || ! -f "$DEMO_CP_FILE" ]]; then
    cat >&2 <<EOF
FAIL missing compiled example classpath.

Run these commands from agent-runtime-java/service first:
  ...
  mvn -pl agent-service-demo dependency:build-classpath -Dmdep.outputFile=target/example.classpath
  ...
  javac -d agent-service-demo/target/example-classes -cp "\$EXAMPLE_CP" agent-service-demo/example/health/HealthProbeExample.java agent-service-demo/example/health/HealthL1ProbeExample.java
EOF
    exit 1
  fi
}
...
java_cp() {
  printf '%s:%s:%s:%s:%s:%s:%s' \
    ...
    "$SERVICE_DIR/agent-service-spec/target/classes" \
    "$(cat "$APP_CP_FILE")" \
    "$(cat "$DEMO_CP_FILE")"
}
...
run_default_version() {
  ...
  assert_json "$body" \
    'data["app"] == "default-version-app" and data["version"] == "0.1.0" and data["agent_loaded"] == True' \
