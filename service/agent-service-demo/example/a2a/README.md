...
Agent B 为 Agent C 和 Agent D 分别配置了流式和非流式远端路由。调用方访问 Agent A 时选择的 A2A 响应方式，与 Agent B 调用下游 Agent 时选择的远端路由分别配置。
...
非流式响应的任务 ID 位于 `result.task.id`，任务状态位于 `result.task.status.state`。流式响应由多行 `data:` 事件组成，任务 ID 位于 `result.statusUpdate.taskId`，任务状态位于 `result.statusUpdate.status.state`。
