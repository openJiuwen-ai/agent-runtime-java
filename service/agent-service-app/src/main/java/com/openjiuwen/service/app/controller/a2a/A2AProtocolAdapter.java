...
/**
 * A2A Message → ServeRequest inbound adapter.
 * Outbound (QueryChunk → A2A Part) is handled by {@link ChunkMapper} + AgentEmitter.
 *
 * @since 0.1.0
 */
public class A2AProtocolAdapter {
    ...
    public ServeRequest toServeRequest(A2AMessageContext ctx) {
        ...
        log.info("A2A toServeRequest taskId={} contextId={} conversationId={} textLen={}",
                ctx.getTaskId(), ctx.getContextId(), req.getConversationId(),
