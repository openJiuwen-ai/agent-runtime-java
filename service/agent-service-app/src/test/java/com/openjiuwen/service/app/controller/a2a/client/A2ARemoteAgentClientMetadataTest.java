...
class A2ARemoteAgentClientMetadataTest {
    ...
    void buildSendParamsAppendsNormalizedPartsAfterLeadingTextPart() {
        var call = new RemoteCall("remote", "analyze", "ctx", null, Map.of(), Map.of(), false,
                List.of(Map.of("kind", "url", "url", "https://example.com/report.pdf", "filename", "report.pdf",
                        "mediaType", "application/pdf"),
                        Map.of("kind", "raw", "bytesBase64", "aGVsbG8=", "byteSize", 5, "filename", "doc.txt",
    ...
    @Test
    void buildSendParamsKeepsUrlFormatAndMovesAdditionalTextAfterFileParts() {
        var call = new RemoteCall("remote", "analyze", "ctx", null, Map.of(), Map.of(), false, List.of(
                Map.of("kind", "text", "text", "preface"),
                Map.of("kind", "url", "url", "https://example.com/report.pdf", "mediaType", "application/pdf")));
        ...
        assertThat(parts.get(1)).isInstanceOf(FilePart.class);
        assertThat(((FilePart) parts.get(1)).file()).isInstanceOf(FileWithUri.class);
        assertThat(parts.get(2)).isInstanceOf(TextPart.class);
        assertThat(((TextPart) parts.get(2)).text()).isEqualTo("preface");
    }
    ...
    void outboundPartsSerializeToFlatV100WireFormat() {
        var call = new RemoteCall("remote", "analyze", "ctx", null, Map.of(), Map.of(), false,
                List.of(Map.of("kind", "url", "url", "https://example.com/chart.png", "filename", "chart.png",
                        "mediaType", "image/png"),
