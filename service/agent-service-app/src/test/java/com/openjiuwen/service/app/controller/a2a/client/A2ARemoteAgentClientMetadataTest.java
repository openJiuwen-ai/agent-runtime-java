...
class A2ARemoteAgentClientMetadataTest {
        ...
        @Test
        void buildSendParamsDropsTextPartDuplicatingLeadingMessage() {
                // 与主文本内容相同的 normalized text part 不应追加，避免远端收到重复文本；
                // 内容不同的 text part 仍正常保留
                var call = new RemoteCall("remote", "analyze", "ctx", null, Map.of(), Map.of(), false,
                                List.of(Map.of("kind", "text", "text", "analyze"),
                                                Map.of("kind", "url", "url", "https://example.com/report.pdf",
                ...
                assertThat(parts.get(1)).isInstanceOf(FilePart.class);
                assertThat(parts.get(2)).isInstanceOf(TextPart.class);
                assertThat(assertInstanceOf(TextPart.class, parts.get(2)).text()).isEqualTo("extra context");
        }
